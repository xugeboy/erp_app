...
class PurchaseOrderNotifier extends StateNotifier<PurchaseOrderState> {
  ...
  Future<void> fetchOrders({
    ...
    bool filtersChanged =
        (orderNumber != null && orderNumber != state.currentOrderNumberQuery) ||
    ...
    try {
      ...
      logger.e(
        "Notifier: DioException fetching orders for page $pageNoToFetch",
        error: e,
        stackTrace: s,
      );
      ...
      ); // Clear orders only if it was a page 1 error
    } catch (e, s) {
      logger.e(
        "Notifier: Exception fetching orders for page $pageNoToFetch",
        error: e,
        stackTrace: s,
      );
      state = state.copyWith(
  ...
  Future<void> goToPreviousPage() async {
    if (state.currentPageNo <= 1 || state.listState == ScreenState.loading)
      return;
    logger.d(
      "Notifier: Going to previous page. Current: ${state.currentPageNo}",
    );
    await fetchOrders(
      pageNoToFetch: state.currentPageNo - 1,
      // Filters are taken from current state
  ...
  Future<void> fetchOrderDetail(int orderId) async {
    ...
    try {
      ...
      logger.e(
        "Notifier: DioException fetching order detail for $orderId",
        error: e,
        stackTrace: s,
      );
      state = state.copyWith(
        detailState: ScreenState.error,
        detailErrorMessage: "网络错误: ${e.message}",
      );
    } catch (e, s) {
      logger.e(
        "Notifier: Exception fetching order detail for $orderId",
        error: e,
        stackTrace: s,
      );
      state = state.copyWith(
  ...
  static const int API_STATUS_APPROVED_AFTER_AUDIT =
      30; // Example: "已审批" or your actual "Approved" state
  ...
  Future<bool> approveOrder(int orderId) async {
    ...
    return _updateOrderStatus(
      orderId,
      API_STATUS_APPROVED_AFTER_AUDIT,
      useAuditEndpoint: true,
    );
  }
  ...
  Future<bool> _updateOrderStatus(
    ...
    state = state.copyWith(
      approvalState: ScreenState.submitting,
      approvalMessage: '',
    );
    try {
      ...
      state = state.copyWith(
        approvalState: ScreenState.success,
        approvalMessage: '订单状态更新成功!',
      );
      ...
      logger.e(
        "Notifier: DioException updating status for order $orderId",
        error: e,
        stackTrace: s,
      );
      state = state.copyWith(
        approvalState: ScreenState.error,
        approvalMessage: "网络错误: ${e.message}",
      );
      return false;
    } catch (e, s) {
      logger.e(
        "Notifier: Exception updating status for order $orderId",
        error: e,
        stackTrace: s,
      );
      state = state.copyWith(
  ...
  void resetApprovalStatus() {
    state = state.copyWith(
      approvalState: ScreenState.initial,
      approvalMessage: '',
    );
  }
}
