...
class _PurchaseOrderListPageState extends ConsumerState<PurchaseOrderListPage> {
  ...
  void initState() {
    ...
    _statusOptions = [
      "全部",
      ..._statusDisplayToCodeMap.keys,
    ]; // "全部" means no status filter
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final initialStatusCode = ref.read(purchaseOrderStatusFilterProvider);
      _searchController.text = initialNotifierState.currentOrderNumberQuery;

      logger.d(
        "PurchaseOrderListPage: Initializing with query: '${_searchController.text}', status code: $initialStatusCode",
      );
      ref
  ...
  Widget build(BuildContext context) {
    ...
    final purchaseOrderNotifier = ref.read(
      purchaseOrderNotifierProvider.notifier,
    );
    final selectedStatusCode = ref.watch(purchaseOrderStatusFilterProvider);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed:
                purchaseOrderState.listState == ScreenState.loading
                    ? null
                    : () {
                      logger.d(
      ...
      body: Column(
        children: [
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      onChanged: (String? newValue) {
                        ...
                        ref
                            .read(purchaseOrderStatusFilterProvider.notifier)
                            .state = nextStatusCode;
                        logger.d(
                          "Status filter changed to: $newValue (Code: $nextStatusCode)",
                        );
                        purchaseOrderNotifier.applyStatusFilter(nextStatusCode);
                      },
                    ),
                  ),
                ),
          ...
          Expanded(
            child: () {
              if (purchaseOrderState.listState == ScreenState.loading &&
                  purchaseOrderState.orders.isEmpty) {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '加载失败: ${purchaseOrderState.listErrorMessage}',
                      ...
                      onTap: () {
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (_) =>
                                    PurchaseOrderDetailPage(orderId: order.id),
                          ),
                        );
                      },
                    ),
          ...
          // --- 分页控制按钮 ---
          if (purchaseOrderState.listState == ScreenState.loaded ||
              purchaseOrderState.orders.isNotEmpty) // 只有在加载完成或列表不为空时显示分页
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ElevatedButton.icon(
                    ...
                    onPressed:
                        (purchaseOrderState.currentPageNo <= 1 ||
                                purchaseOrderState.listState ==
                                    ScreenState.loading)
                  ...
                  Padding(
                    ...
                    child: Text('第 ${purchaseOrderState.currentPageNo} 页'),
                  ),
                  ElevatedButton.icon(
                    icon: const Icon(Icons.arrow_forward_ios),
                    label: const Text("下一页"),
                    onPressed:
                        (!purchaseOrderState.canLoadMore ||
                                purchaseOrderState.listState ==
                                    ScreenState.loading)
