...
class PurchaseOrderRemoteDataSourceImpl
  ...
  final String _getPurchaseOrdersUrl =
      'http://192.168.0.158:48080/admin-api/erp/purchase-order/page';
  final String _getPurchaseOrderDetailBaseUrl =
      'http://192.168.0.158:48080/admin-api/erp/purchase-order/get';
  final String _updatePurchaseOrderStatusBaseUrl =
      'http://192.168.0.158:48080/admin-api/erp/purchase-order/update-status';
  final String _aduitBaseUrl =
      'http://192.168.0.158:48080/admin-api/erp/purchase-order/audit';
  ...
  Future<PaginatedOrdersResult> getPurchaseOrders({
    ...
    try {
      ...
      logger.e(
        "DataSource: Unexpected error in getPurchaseOrders (page $pageNo)",
        error: e,
        stackTrace: s,
      );
      throw Exception(
        'Data source error in getPurchaseOrders: ${e.toString()}',
      );
    }
  }
  ...
  Future<PurchaseOrderModel> getPurchaseOrderDetail(int orderId) async {
    ...
    try {
      ...
      logger.e(
        "Unexpected error in getPurchaseOrderDetail: $e",
  ...
  Future<void> updatePurchaseOrderStatus(
    ...
    final String requestUrl =
        useAuditEndpoint ? _aduitBaseUrl : _updatePurchaseOrderStatusBaseUrl;

    logger.d(
      'Attempting to update purchase order status for $orderId to $newStatus at $requestUrl',
    );
    ...
    try {
      ...
      logger.d(
        "DioException in updatePurchaseOrderStatus (${useAuditEndpoint ? 'audit' : 'update-status'}): ${e.response?.statusCode} - ${e.response?.data ?? e.message}",
      );
      rethrow;
    } catch (e) {
      logger.e(
        "Unexpected error in updatePurchaseOrderStatus: $e",
        stackTrace: (e is Error ? e.stackTrace : null),
      );
